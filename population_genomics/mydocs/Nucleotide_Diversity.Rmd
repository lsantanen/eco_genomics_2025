...
```{r}
theta <- read.table("2103_ALL_win50000_step50000.thetas", sep="\t", header=T)
head(theta)
```
...
ggplot(theta2, aes(nSites)) + 
  geom_histogram(bin=50, fill = "light blue", color="black") +
...
ggplot(theta2, aes(x=Chr, y=tPsite, color=Chr)) +
  ...
  ylim(0,0.07) +
  labs(title="Nucleotide diversity in 50 kb windows for population 2103", x="Scaffold", y="Theta-Pi")
```
...
ggplot(theta2, aes(x=tWsite, y=tPsite, color=Chr)) +
  ...
  labs(title="Pairwise differences vs. segregating sites for population 2103", x="Theta-W",y="Theta-Pi")
