...
library(qqman)
```
Re-make the PCA on just the red spruce samples after removing the black spruce samples
...
#1:95 represents the number of RS samples
names(pops) = c("Ind", "Pop", "Row", "Col")
```

Make the PCA on just red spruce samples 
...
Test for correlation between Admixture scores from the red-black spruce analysis and the PC1 scores from the red spruce only analysis. Is PC1 still an axis of introgression? Is there any evidence for introgression also along PC2?
...
ggarrange(p1,p2)
```
Now, read in our selection stats and calculate the p-values based on the Chi-squared distribution 
...
Now make a Manhattan plot for PC1 and PC2 separately:
```{r}
